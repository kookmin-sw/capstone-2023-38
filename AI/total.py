...
def append_to_musinsa_json(category, id):
    ...
    arr = [362, 1035, 263, 1693, 4054]
    new_data['items'][0]['categoryid'] = arr[int(category)]
    new_data['set_id'] = str(find_num())
...
def file_save(request):
    try:
        ...
        append_to_musinsa_json(predicted.item(), folder_num)

        print(response)

        command2 = "./extract_feature.sh"
